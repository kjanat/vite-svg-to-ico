import { inputBasename } from '#loadInput';
import { isLegacyEmit } from '#types';
import type { EmitSpec, IconSize, IcoSpec, NormalizedEmit, PluginOptions, PngSpec, SvgSpec } from '#types';
import type { LegacyEmitOptions } from '#types'; // v2 compat
...
function legacyToSpecs(legacy: LegacyEmitOptions, defaults: Defaults): EmitSpec[] {
  ...
  if (sourceOpt !== undefined && sourceOpt !== false) {
    ...
    if (enabled) {
      const svg: SvgSpec = {
        format: 'svg',
        filename: typeof sourceOpt === 'object' && sourceOpt.name ? sourceOpt.name : defaults.defaultSvgFilename,
        inject: wantsInjectMinimal,
      };
      specs.push(svg);
    }
