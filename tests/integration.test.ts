...
describe('integration: dev option', () => {
	...
	it('dev: { injection: "shim" } injects shim script instead of link tags', async () => {
		const srv = await createServer({
			...
			server: { port: 0, strictPort: false },
			plugins: [
				svgToIco({ input: 'icon.svg', emit: { inject: 'minimal' }, dev: { injection: 'shim' } }),
			],
		});
		await srv.listen();
		try {
	...
	it('dev: { hmr: false } does not inject HMR script', async () => {
		const srv = await createServer({
			...
			server: { port: 0, strictPort: false },
			plugins: [
				svgToIco({ input: 'icon.svg', emit: { inject: 'minimal' }, dev: { hmr: false } }),
			],
		});
		await srv.listen();
		try {
	...
	it('dev: { injection: "shim", hmr: false } shim has no HMR code', async () => {
		const srv = await createServer({
			...
			server: { port: 0, strictPort: false },
			plugins: [
				svgToIco({ input: 'icon.svg', emit: { inject: 'minimal' }, dev: { injection: 'shim', hmr: false } }),
			],
		});
		await srv.listen();
		try {
