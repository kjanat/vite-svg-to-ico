...
import { invalid, unwrap } from '#testHelpers';
...
describe('svgToIco plugin factory', () => {
	...
	it('accepts a file:// URL instance as input', () => {
		const url = Bun.pathToFileURL(FIXTURE);
...
describe('v3 EmitSpec normalization', () => {
	...
	it('rejects PngSpec inject.sizes not subset of spec.sizes', () => {
		const { logger } = captureLogger();
		expect(() =>
			runConfig({ input: FIXTURE, emit: [{ format: 'png', sizes: [16, 32], inject: { sizes: [64] } }] }, logger)
		).toThrow('subset');
	});
	...
	it('accepts PngSpec inject.sizes that is a valid subset', () => {
		const { logger } = captureLogger();
		expect(() =>
			runConfig({ input: FIXTURE, emit: [{ format: 'png', sizes: [16, 32, 64], inject: { sizes: [32] } }] }, logger)
		).not.toThrow();
	});
	...
	it('rejects spec sizes out of range', () => {
		const { logger } = captureLogger();
		expect(() => runConfig({ input: FIXTURE, emit: [{ format: 'ico', sizes: invalid<IconSize[]>([0, 500]) }] }, logger))
	...
	it('accepts PngSpec sizes above the ICO 256 cap', () => {
		...
		const { logger } = captureLogger();
		expect(() => runConfig({ input: FIXTURE, emit: [{ format: 'png', sizes: [192, 512, 1024] }] }, logger)).not
			.toThrow();
	});

	it('rejects PngSpec sizes above the 4096 sanity cap', () => {
		const { logger } = captureLogger();
		expect(() => runConfig({ input: FIXTURE, emit: [{ format: 'png', sizes: invalid<IconSize[]>([5120]) }] }, logger))
	...
	it('rejects unknown format', () => {
		const { logger } = captureLogger();
		expect(() =>
			runConfig({ input: FIXTURE, emit: invalid<PluginOptions['emit']>([{ format: 'bmp', sizes: [16] }]) }, logger)
		).toThrow('invalid');
	});
});
