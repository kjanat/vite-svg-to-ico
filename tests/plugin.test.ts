...
describe('v3 EmitSpec normalization', () => {
  ...
  it('rejects spec sizes out of range', () => {
    const { logger } = captureLogger();
    expect(() =>
      runConfig({ input: FIXTURE, emit: [{ format: 'ico', sizes: invalid<IconSize[]>([0, 500]) }] }, logger),
  ...
  it('accepts PngSpec sizes above the ICO 256 cap', () => {
    ...
    const { logger } = captureLogger();
    expect(() =>
      runConfig({ input: FIXTURE, emit: [{ format: 'png', sizes: [192, 512, 1024] }] }, logger),
    ).not.toThrow();
  });
  ...
  it('rejects PngSpec sizes above the 4096 sanity cap', () => {
    const { logger } = captureLogger();
    expect(() =>
      runConfig({ input: FIXTURE, emit: [{ format: 'png', sizes: invalid<IconSize[]>([5120]) }] }, logger),
...
describe('config plugin validation', () => {
  ...
  it('throws on invalid dev.injection string', () => {
    expect(() =>
      runConfigResolved({ input: 'icon.svg', dev: invalid<PluginOptions['dev']>({ injection: 'bad' }) }),
